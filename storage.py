...
local files for development:
  ...
  collections  one JSON document per feature (journal, garden, music catalog,
               blog, messages, portfolio uploads, garden photos, accounts
               template, travel, page settings)
...
def _read_file(name):
    ...
    for path in [DATA_DIR / filename] + ([MOCK_DIR / mock] if mock else []):
        try:
            if path.exists():
